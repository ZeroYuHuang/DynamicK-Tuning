...
from topk_models import TopKLlamaConfig, TopKLlamaForCausalLM


# wandb.login(key="e14efd86123a4685ec03482f998bac96362853bd")
...
class ModelArguments:
    ...
    base_config_dir: Optional[str] = field(default="./7b_base_configs.json")
    split_start_layer: int = field(default=16)
    split_every_layer: int = field(default=2)
    topk: int = field(default=64)
    mode: str = field(default="topk")
    select: str = field(default="gate")
    dynamic_mode: str = field(default="static")
...
def train():
    ...
    # config 
    config = TopKLlamaConfig.from_json_file(
    ...
    added_name = f"{model_name}-{model_args.select}-{model_args.split_start_layer}-{model_args.split_every_layer}_"
    if model_args.dynamic_mode == 'static':
        added_name += f"topk_{model_args.topk}"
    else:
        added_name += f"{model_args.dynamic_mode}_threshold_{model_args.threshold}"
